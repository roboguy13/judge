...
data Judge term s j a where
  V :: a -> Judge term s j a

  Lam :: Judge term s j a -> (a -> Judge term s j a) -> Judge term s j a
  App :: Judge term s j a -> Judge term s j a -> Judge term s j a

  Forall :: Judge term s j a -> (a -> Judge term s j a) -> Judge term s j a
  (:@) :: Judge term s j a -> Judge term s j a -> Judge term s j a

    -- | Lifting a j from the object language into the metalanguage
  Lifted :: j a -> Judge term s j a
  -- Lifted :: j (MetaVar a b) -> Judge term s j a
  Universe :: Int -> Judge term s j a
  -- Const :: s -> Judge term s j a

  TermLift :: j (term a) -> Judge term s j a

pattern Set = Universe 0
