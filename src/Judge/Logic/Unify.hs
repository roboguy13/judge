...
data UnifyPair = forall t. UnifyC t => UnifyPair t t

class (Alpha t, Typeable t, Subst t t) => Unify t where
  mkVar :: Name t -> t

  matchOne :: Fresh m => t -> t -> m (Maybe [UnifyPair]) -- | If the constructors match, give back the children for each

    -- The Fresh m is for generating fresh names for binders
  getChildren :: Fresh m => t -> m [t]

  default matchOne :: (Generic t, Ppr t, GConstrEq (Rep t), Fresh m) => t -> t -> m (Maybe [UnifyPair])
  matchOne x y =
...
data AName t a where
  AName :: (Typeable a, Subst a t) => Name a -> AName t a
...
--
-- unifyList :: forall t. (Ppr t, UnifyC t, Plated t) => Substitution t -> [UnifyPair] -> FreshMT Maybe (Substitution t)
